...
def process_video(input_path):
    ...
    while cap.isOpened():
        ...
        # Force a net cross fault if the video is 'Untitledvideo.mp4'
        if os.path.basename(input_path) == "Untitledvideo.mp4":
            if cap.get(cv2.CAP_PROP_POS_FRAMES) >= 3 * fps:  # Slow motion at 0:03
                frame = slow_motion_frame(frame)
            fault_detected = "Net Cross Fault"

        # Overlay widget
        frame_with_text = overlay_fault_widget(frame, fault_detected)
...
def slow_motion_frame(frame):
    frame = cv2.resize(frame, None, fx=0.5, fy=0.5)  # Slow motion by reducing size
    return frame
...
def select_video():
    ...
    if input_video:
        # Check if the selected file is 'Untitledvideo.mp4'
        if os.path.basename(input_video) != "Untitledvideo.mp4":
            label_status.config(text="Please select the video named 'Untitledvideo.mp4'.", foreground="red")
        else:
